...
pub async fn run(
    ...
    let background = match job.background {
        Some(background) => Some(write_background(job_id, background).await?),
        None => None,
    };
    let result = run_worker(&runtime, job_id, cancel, &job, &root, background.as_deref()).await;
    if let Some(background) = background {
        let _removed = tokio::fs::remove_file(&background).await;
    }
    let minutes = result?;
...
async fn write_background(job_id: Uuid, background: &str) -> Result<PathBuf, AppError> {
    let path = std::env::temp_dir().join(format!("galpi-background-{job_id}.md"));
    tokio::fs::write(&path, background)
        .await
