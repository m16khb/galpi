...
1. Galpi를 실행합니다.
2. 화자분리 모델 접근 승인이 필요한 경우 아래 방식으로 Hugging Face 토큰을 입력합니다.
3. **로컬 엔진 준비**를 누릅니다.
4. 엔진, 모델, ffmpeg가 모두 `준비됨`으로 표시될 때까지 기다립니다.

최초 준비는 약 3GB의 모델을 내려받을 수 있습니다. 토큰은 프로세스 환경으로만 전달되며 앱에 저장하지 않습니다. 준비가 끝난 뒤에는 같은 앱 데이터 폴더와 모델 캐시를 재사용합니다.
...
6. `hf_`로 시작하는 토큰을 Galpi의 입력란에 붙여넣고 로컬 엔진 준비를 시작합니다.

토큰은 모델 준비 프로세스에만 전달되고 입력란에서도 즉시 지워집니다. 각 Mac은 앱 전용 모델 캐시를 별도로 사용하므로, 새 Mac의 최초 준비에서는 같은 절차가 다시 필요할 수 있습니다.
